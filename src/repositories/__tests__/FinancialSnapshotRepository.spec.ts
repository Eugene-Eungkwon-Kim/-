...
describe('FinancialSnapshotRepository (Day 5 - Task 4: 금융 지표 저장소, δ=1255)', () => {
  ...
  beforeEach(async () => {
    ...
    snapshots = new FinancialSnapshotRepository(pool);
    const user = await users.register({ email: 'snapshot@example.com', name: 'Snapshot User', password: 'test-pass', contact: { address: {} } });
    userId = user.id;
  });
  ...
  describe('[T-API-A19~A24] 금융 지표 저장소', () => {
    it('[T-API-A19] 재정 스냅샷 저장 (upsert)', async () => {
      const created = await snapshots.recordSnapshot(baseSnapshot());
      expect(created.creditScore).toBe(750);

      // 같은 날짜로 재기록하면 갱신될 뿐 중복 생성되지 않는다
      const updated = await snapshots.recordSnapshot(baseSnapshot({ creditScore: 770 }));
      expect(updated.id).toBe(created.id);
      expect(updated.creditScore).toBe(770);

      const history = await snapshots.getHistory(userId);
      expect(history.length).toBe(1);
    });

    it('[T-API-A20] 히스토리 조회 (기간 필터)', async () => {
      await snapshots.recordSnapshot(baseSnapshot({ snapshotDate: '2026-01-01' }));
      await snapshots.recordSnapshot(baseSnapshot({ snapshotDate: '2026-02-01' }));
      await snapshots.recordSnapshot(baseSnapshot({ snapshotDate: '2026-03-01' }));

      const all = await snapshots.getHistory(userId);
      expect(all.map((s) => s.snapshotDate)).toEqual(['2026-01-01', '2026-02-01', '2026-03-01']);

      const filtered = await snapshots.getHistory(userId, '2026-02-01', '2026-02-28');
      expect(filtered.length).toBe(1);
      expect(filtered[0].snapshotDate).toBe('2026-02-01');
    });

    it('[T-API-A21] 시계열 분석 (신용점수 추세)', async () => {
      await snapshots.recordSnapshot(baseSnapshot({ snapshotDate: '2026-01-01', creditScore: 700 }));
      await snapshots.recordSnapshot(baseSnapshot({ snapshotDate: '2026-02-01', creditScore: 730 }));
      await snapshots.recordSnapshot(baseSnapshot({ snapshotDate: '2026-03-01', creditScore: 760 }));

      const trend = await snapshots.getTrend(userId, 'creditScore');
      expect(trend.points.map((p) => p.value)).toEqual([700, 730, 760]);
      expect(trend.direction).toBe('improving');
      expect(trend.changeFromFirst).toBe(60);
    });

    it('[T-API-A22] 추세 분석 & 예측 (리스크 점수는 낮을수록 개선)', async () => {
      await snapshots.recordSnapshot(baseSnapshot({ snapshotDate: '2026-01-01', riskScore: 20 }));
      await snapshots.recordSnapshot(baseSnapshot({ snapshotDate: '2026-02-01', riskScore: 35 }));
      await snapshots.recordSnapshot(baseSnapshot({ snapshotDate: '2026-03-01', riskScore: 55 }));

      // riskScore가 상승했다는 것은 위험이 커졌다는 뜻 → declining으로 해석되어야 한다
      const riskTrend = await snapshots.getTrend(userId, 'riskScore');
      expect(riskTrend.direction).toBe('declining');

      const surplusTrend = await snapshots.getTrend(userId, 'monthlySurplus');
      expect(surplusTrend.direction).toBe('stable'); // 매 스냅샷 동일값(1,000,000)
    });

    it('[T-API-A23] 임계값 경고', async () => {
      await snapshots.recordSnapshot(
        baseSnapshot({ creditScore: 450, financialHealthScore: 35, riskScore: 45, debtToIncomeRatio: 0.9 })
      );

      const alerts = await snapshots.checkThresholds(userId);
    ...
    it('[T-API-A24] 성과 비교 (기간 간 지표 변화)', async () => {
      await snapshots.recordSnapshot(
      ...
      const comparison = await snapshots.comparePerformance(userId, '2026-01-01', '2026-06-01');
