...
describe('financialAnalyticsService (Day 6 - Task 5: 금융분석 서비스 계층, δ=1350)', () => {
  ...
  describe('[T-SVC-501~506] 금융분석 서비스', () => {
    it('[T-SVC-501] 신용시뮬레이션이 DB의 현재 신용점수를 시작점으로 사용한다', async () => {
      const result = await runCreditSimulation(pool, userId, { scenario: 'ideal', duration: 6 });
    ...
    it('[T-SVC-502] 존재하지 않는 사용자', async () => {
      const result = await runCreditSimulation(pool, 'no-such-user', { scenario: 'normal' });
      expect(result.success).toBe(false);
      if (!result.success) expect(result.error.code).toBe('USER_NOT_FOUND');
    });

    it('[T-SVC-503] 리스크평가가 DB의 소득/부채/자산을 사용한다', async () => {
      const result = await runRiskAssessment(pool, userId, '2026-01-01');
      expect(result.success).toBe(true);
      if (result.success) {
    ...
    it('[T-SVC-504] 리스크평가 결과가 스냅샷으로 자동 저장된다', async () => {
      await runRiskAssessment(pool, userId, '2026-01-01');

      const snapshot = await new FinancialSnapshotRepository(pool).getSnapshotByDate(userId, '2026-01-01');
    ...
    it('[T-SVC-506] 여러 시점의 리스크평가가 시계열 추세를 형성한다', async () => {
      ...
      await updateUserProfile(pool, userId, { creditProfile: { score: 820 } }, 1);
      await runRiskAssessment(pool, userId, '2026-02-01');
  ...
  describe('[T-SVC-801~804] 재정분석 서비스 연결 (Day 7 - Task 3, δ=1105)', () => {
    it('[T-SVC-801] DB의 실제 income/expenses/debt/assets를 사용한다', async () => {
      const result = await runFinancialAnalysis(pool, userId, '2026-01-01');
      expect(result.success).toBe(true);
      if (result.success) {
    ...
    it('[T-SVC-802] 존재하지 않는 사용자', async () => {
      const result = await runFinancialAnalysis(pool, 'no-such-user', '2026-01-01');
      expect(result.success).toBe(false);
      if (!result.success) expect(result.error.code).toBe('USER_NOT_FOUND');
    });

    it('[T-SVC-803] 결과가 스냅샷으로 자동 저장된다', async () => {
      await runFinancialAnalysis(pool, userId, '2026-01-01');
    ...
    it('[T-SVC-804] 기존 리스크평가 스냅샷의 risk 필드를 재정분석이 덮어쓰지 않는다', async () => {
      const risk = await runRiskAssessment(pool, userId, '2026-01-01');
      expect(risk.success).toBe(true);
      const riskScoreBefore = (await new FinancialSnapshotRepository(pool).getSnapshotByDate(userId, '2026-01-01'))?.riskScore;

      // 같은 날짜에 재정분석을 실행해도(리스크 계산을 하지 않는 분석) 기존 riskScore가 유지되어야 한다
      await runFinancialAnalysis(pool, userId, '2026-01-01');
      const riskScoreAfter = (await new FinancialSnapshotRepository(pool).getSnapshotByDate(userId, '2026-01-01'))?.riskScore;
