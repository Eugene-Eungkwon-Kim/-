...
export const loanHandlers = [
  ...
  rest.post('/api/v1/planning/financial-analysis', async (req, res, ctx) => {
    const body = await req.json() as any;
    // Day7: 이 핸들러는 src/services/__tests__/handlers.spec.ts의 로직과도 어긋난
    // 독립된 세 번째 구현이었다(건강도 가중치, 등급 구간이 서로 달랐음).
    // src/services/financialAnalysis.ts로 추출된 단일 정본에 위임해 통합한다.
    return res(ctx.json(await analyzeFinancials(body)));
  }),
