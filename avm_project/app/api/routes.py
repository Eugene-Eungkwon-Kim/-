...
class AVMDiagnostics(BaseModel):
    status: str
...
class AVMResponse(BaseModel):
    ...
    diagnostics: Optional[AVMDiagnostics] = None


class PrecedentItem(BaseModel):
    property_serial: Optional[str]
...
class AuctionStatItem(BaseModel):
    result: Optional[str]
    count: int
    avg_hammer_rate: Optional[float]


class AuctionStatsResponse(BaseModel):
    property_type: str
    address_sido: Optional[str]
    stats: list[AuctionStatItem]
...
class ComparableSaleItem(BaseModel):
    subject_serial: Optional[str]
    address: Optional[str]
    property_type: Optional[str]
    use_zone: Optional[str]
    land_area: Optional[float]
    building_area: Optional[float]
    ...
    unit_price_py: Optional[float]
    land_price_sqm: Optional[float]
    note: Optional[str]


class ComparableSalesFilter(BaseModel):
    sido: str
...
class ComparableSalesResponse(BaseModel):
    count: int
    filtered_by: ComparableSalesFilter
    items: list[ComparableSaleItem]
...
@router.get("/precedents", response_model=PrecedentsResponse, summary="유사 물건 전례 조회")
def get_precedents(
...
@router.get("/auction-stats", response_model=AuctionStatsResponse, summary="낙찰가율 통계")
...
@router.get("/properties/{property_serial}", response_model=PropertyDetailResponse, summary="물건 상세 조회")
...
@router.get("/comparable-sales", response_model=ComparableSalesResponse, summary="거래사례(정밀시트) 조회")
