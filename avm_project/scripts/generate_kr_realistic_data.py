...
def _compute_price(
    ...
    area: float, floor: int, year: int,
    rng: np.random.Generator,
) -> Tuple[float, float]:
    """(new_price, old_price) — 비선형 가격 공식."""
    ...
    age_factor    = max(0.60, 1.0 - 0.012 * (2026 - year))
    noise         = float(rng.lognormal(mean=0.0, sigma=0.06))

    new_price = price_per_sqm * area * location_factor * floor_factor * age_factor * noise
    new_price = max(50_000_000.0, min(6_000_000_000.0, new_price))
    old_price = new_price * float(rng.uniform(0.88, 0.98))
    return round(new_price, -4), round(old_price, -4)
...
def generate_dataset(n_rows: int = 10_000, seed: int = 42) -> pd.DataFrame:
    ...
    for i in range(n_rows):
        ...
        month = months[i % 12]
        new_price, old_price = _compute_price(region, lat, lng, area, floor, year, rng)
...
def validate_and_report(df: pd.DataFrame) -> None:
    ...
    assert (df['old_price'] < df['new_price'] * 1.05).all(), "old_price > new_price"
